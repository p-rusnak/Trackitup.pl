    ...
    switch (sort) {
        ...
        case 'grade':
            if (details[mode][diff]) {
                return compareGrades(details[mode][diff][ar[0]]?.grade, details[mode][diff][br[0]]?.grade)
            } else {
                return -1
            }
        case 'fav':
...
const filterItems = (a, details, mode, diff, hidden, hideScore, tags) => {
    ...
    const grade = (details[mode][diff] && details[mode][diff][a[0]]?.grade) || undefined
    if (hidden.pass) {
        if (grade) show = false
...
const Songs = ({mode}) => {
    ...
    useEffect(() => {
        ...
        if(search?.p1Diff && search?.p2Diff) {
            newSongList = newSongList.filter(([key, value]) =>{
                ...
                return (
                    !!value.diffs?.find(d => d.type === mode && d.diff === prefix1 + search.p1Diff) && 
                    !!value.diffs?.find(d => d.type === mode && d.diff === prefix2 + search.p2Diff)
                )
            })
        }
    ...
    const changeGrade = (value) => {
        apiClient.postScores(mode, { songId: openChart.id, diff: openChart.diff, grade: value })
    ...
    const shouldDisplayDiff = (diff) => {
        ...
        if (search && search.p1Diff &&  search.p2Diff)
            if (!(diff === `${prefix1}${search.p1Diff}`) && !(diff === `${prefix2}${search.p2Diff}`))
                result = false

        if(!data[diff]?.some(item => 
            ...
            <div>
                ...
                <Accordion>
                    ...
                    <AccordionDetailsStyled>
                        ...
                        <DiffSearch>
                            With diffs: 
                                <NumberInput
                                    ...
                                    max="28"
                                    value={search?.p1Diff || ''}
                                    onChange={(e) => setSearch(e.target.value ? { ...search, p1Diff: e.target.value } : undefined)}
                                />
                                <NumberInput
                                    label='P2'
                                    type="number"
                                    ...
                                    max="28"
                                    value={search?.p2Diff || ''}
                                    onChange={(e) => setSearch(e.target.value ? { ...search, p2Diff: e.target.value } : undefined)}
                                />
                                <Button onClick={() => setSearch({...search, p1Diff: undefined, p2Diff: undefined})}>Clear</Button>
                        </DiffSearch>
                        <div>
                            Tags (not everything is included): 
                                <FormControlLabel control={<Checkbox onChange={(_, b) => setTags({ ...tags, side: b })} value={tags.side} />} label="Side" />
                                <FormControlLabel control={<Checkbox onChange={(_, b) => setTags({ ...tags, twist: b })} value={tags.twist} />} label="Twist" />
                                <FormControlLabel control={<Checkbox onChange={(_, b) => setTags({ ...tags, drill: b })} value={tags.drill} />} label="Drill" />
                        ...
                        <div>
                                Hide:
                                <FormControlLabel control={<Checkbox onChange={(_, b) => {setHidden({ ...hidden, pass: b })}} value={hidden.pass} />} label="Pass" />
                                <FormControlLabel control={<Checkbox onChange={(_, b) => {setHidden({ ...hidden, played: b }) }} value={hidden.played} />} label="Played" />
                ...
                {Object.entries(diffCounter[mode]).map(([diff, count]) => 
                    shouldDisplayDiff(diff) &&
                        ...
                        >
                            ...
                            >
                                <Typography><DiffBall className={`${mode} ${diff}`}>{`(${(details[mode][diff] && Object.values(details[mode][diff]).filter(v => !!v.grade)?.length) || 0}/${count}) `}</DiffBall></Typography>
                            </AccordionSummary>
                            <AccordionDetailsStyled>
                                {(
                                data[diff]
                                    ?.filter(a => filterItems(a, details, mode, diff, hidden, hideScore, tags))
                                    .sort((a, b) => sortByFilters(a, b, diff, details, mode, sort))
                                    .map(chart => {
                                    const chartData = {
                                        id: chart[0],
                                        ...chart[1],
                                        mode,
                                        diff,
                                        ...(details[mode][diff] ? details[mode][diff][chart[0]] : {})
                                    } 

                                    const adiff = chartData.diffs.find(a => a.diff === diff && a.type === mode)?.adiff
                                    ...
                                    prevChart = adiff 
                                        ...
                                        return <React.Fragment
                                            key={`${chart[0]}-${diff}`}
                                            >
                                            {divider && <DividerStyled>{adiff || 'Not Specified'}</DividerStyled>}
                                            <Thumbnail
                                                data={chartData}
